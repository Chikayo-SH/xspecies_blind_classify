...
%% Path
% addDirPrefs_COS;  % ← もう使わない

dirPref = getpref("cosProject","dirPref");

% 解析結果を保存する先（human/376 配下）
save_dir = fullfile(dirPref.rootDir, "preprocessed", species, subject);

% detectChannels_*.mat が置いてある場所（preprocessed 直下）
detect_dir = fullfile(dirPref.rootDir, "preprocessed");

% 生データの場所
load_dir = fullfile(dirPref.rawDir, "human", "Stage1");
...
% detectChannels_376.mat は preprocessed 直下にあるので detect_dir を使う
load(fullfile(detect_dir, "detectChannels_" + subject), ...
     "channel","tgtChannels","lobe");
...
for ich = 1:numel(tgtChannels)
    ...
    data_raw(:,:,2) = data_tmp{2}(:,1:minTrials);
        ...
        % awake 条件 (condition 1) について
    % raw / mean subtraction / mean+line noise を一枚に図示
...
% ここを修正
for icond = 1:2
    this_data = data_raw(:,:,icond);  % 条件ごとの data: time x trial

    [data_proc(:,:,icond), preprocess_string, ...
        powers_before(:,:,icond), powers_after(:,:,icond), ...
        faxis_before, faxis_after] = ...
        preprocessOneCh(this_data, params, s_subtractMean, s_lineNoise);
end
    ...
    % 図を PNG で保存（screen2png の代わり）
    figFile = fullfile(save_dir, [savedata_prefix, '_powerspectra.png']);
    try
        exportgraphics(gcf, figFile, "Resolution", 150);  % R2020a以降
    catch
        saveas(gcf, figFile);  % 何かあっても最低限保存されるように
    end
