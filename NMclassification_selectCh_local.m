...
% ★ここで param を自前で定義する
param.ncv   = 10;    % cross-validation の分割数
param.alpha = 0.05;  % 有意水準
param.q     = 0.05;  % FDR の q

dirPref = getpref('cosProject','dirPref');

hctsaType = 'TS_Normalised';  % 0–1 スケールの HCTSA を使う
...
% ★Fig2A と同じく正規化済み HCTSA を使う
hctsaType = 'TS_Normalised'; % 'TS_DataMat';

preprocessSuffix = '_subtractMean_removeLineNoise';
svm = false;

% ★train = macaque George, validate = human 376
species_train    = 'macaque';
subject_train    = 'George';
species_validate = 'human';
subject_validate = '376';
...
load(fullfile(load_dir_train,['detectChannels_' subject_train]) ,'tgtChannels');
tgtChannels_train_all = tgtChannels;

load(fullfile(load_dir_validate,['detectChannels_' subject_validate]) ,'tgtChannels');
tgtChannels_validate_all = tgtChannels;
clear  tgtChannels;

% ★ここで「実際に使うch」を ch017 / ch157 のみに固定する
tgtChannels_train    = 17;   % George ch017
tgtChannels_validate = 157;  % Human 376 ch157
...
for JID = 1:maxJID
    ...
    try
        ...
        out_file = fullfile(save_dir, sprintf('%s_train_%s_%s_ch%03d_validate_%s_%s_ch%03d_accuracy', ...
            hctsaType, species_train, subject_train, ch_train, species_validate,subject_validate, ch_validate));
...
%         % ★ George ch017 はファイル名に tapers5-19 が入っている
% file_string_train = fullfile(hctsa_dir_train, ...
%     sprintf('%s_%s_ch%03d_tapers5-19_hctsa', species_train, subject_train, ch_train));
% trainData = load([file_string_train '.mat'], ...
%     'Operations', 'TS_DataMat', 'TimeSeries', 'TS_Normalised');
% 
% % ★ human ch157 は v2 というサフィックスつき
...
% validateData = load([file_string_validate '.mat'], ...
%     'Operations', 'TS_DataMat', 'TimeSeries', 'TS_Normalised');
% George ch017 v2
file_string_train = fullfile(hctsa_dir_train, ...
    sprintf('%s_%s_ch%03d_tapers5-19_hctsa_v2', ...
...
% trainFraction: 学習に使う割合（CVするので 1.0 でOK）
%% train nearest-median classifier w cross-validation

% ★ trainFraction / labelToClassify は不要（古い仕様）
% trainFraction    = 1.0;
% labelToClassify  = condNames;

% 本当のシグネチャ: (trainData, validateData, ncv, condNames, hctsaType)
[classifier_cv, fig] = NMclassifier_cv(trainData, validateData, param.ncv, condNames, hctsaType);

set(fig,'Position',[0 0 1000 500]);
% screen2png(out_file, fig);
saveas(fig, [out_file '_NMclassifier_cv.png']);
...
close(fig);
        ...
        order_f = validFeatures(clusterFeatures(trainData.(hctsaType)(:,validFeatures)));
        
        data_all = [trainData.(hctsaType); validateData.(hctsaType) ];
       ...
       %          %% single trial (subjectEpochs が空じゃないときだけ)
       %  if ~isempty(subjectEpochs{1}) && ~isempty(subjectEpochs{2})
       %      fig = figure('position',[0 0 1000 50]);
       %      ax(1)=subplot(121);
       %      imagesc(data_all(subjectEpochs{1}(1),order_f));
       %      ax(2)=subplot(122);
       ...
       %  end
        ...
        % ★TS_Normalisedではなく、元のTS_DataMatを使ってヒストを描く
        data_all_raw = [trainData.TS_DataMat; validateData.TS_DataMat];

        ff_rc = pdensity_awakeUnconscious(data_all_raw, TimeSeries_all, classifier_cv.operations.CodeString, ...
